...
import { fetchTrends } from '../../services/twitterApi';
// const trends = [
...
export default function TrendingTopics() {
  ...
  const fetchData = async () => {
    try {
      const resp = await fetchTrends("general");
      setTrends(resp);
    } catch (error) {
      console.error(error);
    }
      ...
      <div className="space-y-4">
        {trends.map((trend, index) => (
          <div key={index} className="flex items-center justify-between">
            ...
            <span className="text-sm text-gray-600">{trend.cleanedVolume.toLocaleString()} tweets</span>
          </div>
        ))}
      </div>
    </div>
